...
import at.fh.ooe.swe4.collections.model.NMKTreeNode.Split;
...
public class TwoThreeFourTreeSet<T> extends
		AbstractSortedSet<T, NMKTreeNode<T>> implements SortedTreeSet<T> {
	...
	private static class BalanceResult<T> {
		 ...
		 *            the node to be set
		 */
		public void setNode(NMKTreeNode<T> node) {
			this.node = node;
		}
	...
	public static class TreeNodeMetadata<T> {
		...
		public final NMKTreeNode<T> toVisit;
		 ...
		 */
		public TreeNodeMetadata(final NMKTreeNode<T> current) {
		 ...
		 */
		public TreeNodeMetadata(final NMKTreeNode<T> current,
				final NMKTreeNode<T> toVisit, ProcessState process) {
			super();
	...
	public boolean add(T e) {
		...
		if (e != null) {
			/*-- no root present --*/
			if (root == null) {
				root = new NMKTreeNode<T>(e, comparator());
				currentHeight = 1;
	...
	private TreeNodeMetadata<T> calculateTreeNodeMetadata(
			final NMKTreeNode<T> _node, final T _key) {
	 ...
	 */
	private void balanceTree(final NMKTreeNode<T> node,
		...
		NMKTreeNode<T> parent, leftNode, rightNode;
		...
		leftNode = new NMKTreeNode<T>(leftKey, comparator());
		rightNode = new NMKTreeNode<T>(rightKey, comparator());
		// 3. split children if 4 children present
		if (node.getChildrenSize() == 4) {
			// split the children
			Map<Split, SortedSet<NMKTreeNode<T>>> splitChildren = node
					.splitChildren();
			// iterate over head an tail subset
			for (Entry<Split, SortedSet<NMKTreeNode<T>>> entry : splitChildren
					.entrySet()) {
				// Add each node to proper new node
				for (NMKTreeNode<T> child : entry.getValue()) {
					switch (entry.getKey()) {
					case HEAD:
